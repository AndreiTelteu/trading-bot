...
func installStrategyDeployment(t *testing.T, db *gorm.DB, settings map[string]string, state string) {
	...
	spec := validation.ManifestSpec{
		...
		ExecutionSemantics: map[string]string{"fee_bps": "10", "slippage_bps": "5", "timing": "decision", "liquidity": "bounded"}, CapacityStress: validation.CapacityStressPolicy{MaxParticipation: .1, ImpactBpsAtMax: 10, StressMultiplier: 2}, Folds: []validation.Fold{
