...
The legal state sequence is `schema_legacy → research_ingestion → ledger_compare → shared_shadow → parity_accepted → new_paper → paper_observation → research_validation → limited_live`. `research_ingestion` is an audited, non-capital Stage 04 entry point for collecting metadata, historical bars, manifests, and universe snapshots on a fresh install: it requires exact research-only PIT/backtest flags, leaves ledger authority `legacy`, leaves the shared engine `off`, and forbids a candidate or Stage 07 context. No metric auto-advances any stage. First POST the proposed complete envelope to `/api/operations/flags/snapshots`; preparing a content-addressed snapshot grants no authority. Record each completed bounded prerequisite at `/api/operations/cutover/evidence`. Then an authenticated operations/governance administrator posts the transition with the target snapshot, exact parity policy/population, Stage 07 context, and evidence IDs. Caller summaries and parity denominators are rejected:
