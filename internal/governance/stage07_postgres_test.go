...
func persistPassingEvidence(t *testing.T, gormDB *gorm.DB, created time.Time, model *validation.ModelAuthority) (validation.ExperimentManifest, validation.PersistedEvidence) {
	...
	pre, err := validation.NewManifest(spec, created)
	if err != nil {
		t.Fatal(err)
	}
	spec.StudyType, spec.Exploratory, spec.FamilyID = "confirmatory", false, pre.Spec.FamilyID
	holdout, err := validation.NewConfirmatoryHoldoutContract(spec.FamilyID, spec.DatasetDigest, spec.Interval)
	if err != nil {
		t.Fatal(err)
	}
